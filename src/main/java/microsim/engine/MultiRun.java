...
public abstract class MultiRun extends Thread implements EngineListener, ExperimentBuilder {
    ...
    public MultiRun() {
        ...
        // multiRunId = "Run " + SimulationEngine.getRnd().nextInt(); //Ross: Why is
        // this Random?
    ...
    public synchronized void go() {
        ...
        if (counter == 2) { // After the first simulation (which by default copies the input files to the
                            // output folder), check the settings on whether to copy input files to new
                            // folder for each simulation run
            ExperimentManager.getInstance().copyInputFolderStructure = copyInputFolderStructure; // By default do not
                                                                                                 // copy the input
                                                                                                 // folder for each
                                                                                                 // simulation run after
                                                                                                 // the first
                                                                                                 // simulation.
        }
    ...
    /**
     * Sets whether to copy the input files into a new input folder within a new
     * output folder for each simulation run
     * 
     * @param copyInputFolderStructure_ - set to true if wanting a copy of the input
     *                                  files to be stored in the output folder for
     *                                  each simulation run, otherwise set to false
     */
