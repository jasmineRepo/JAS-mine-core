...
public class IterativeSimpleMatching<T> implements IterativeMatchingAlgorithm<T> {
    ...
    public Pair<Set<T>, Set<T>> matching(Collection<T> collection1, Predicate<T> filter1, Comparator<T> comparator1,
        ...
        for (int i = 0; i < c1.size(); i++) { // Now check all agents in c1, because a match does not always occur (if,
            ...
            // Bug in the map below - you can potentially throw away some candidates, if
            // they have the same score as another candidate that comes along afterwards (a
            // map will over-write entries with the same key).
