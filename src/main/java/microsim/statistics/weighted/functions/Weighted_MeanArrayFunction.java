...
/**
 * This class computes the (weighted) average (mean) value of an array of values
 * taken from a data source,
 * weighted by corresponding weights:
 * weighted mean = sum (values * weights) / sum (weights)
 * Note that the array of weights must have the same length as the array of
...
public class Weighted_MeanArrayFunction extends AbstractWeightedArrayFunction implements IDoubleSource {
    ...
    public void apply(double[] data, double[] weights) {
        ...
        // throw new IllegalArgumentException("Error - the length of the array of data
        // does not match the length of the array of weights!");
        // }
