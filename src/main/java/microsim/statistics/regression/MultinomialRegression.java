 ...
 *         Each y_i is an indicator variable for discrete value i, where the
 *         index i signifies nothing
...
public class MultinomialRegression<E1 extends Enum<E1> & IntegerValuedEnum> implements IDiscreteChoiceModel {
    ...
    public MultinomialRegression(RegressionType type, Class<E1> enumType,
            MultiKeyCoefficientMap multinomialCoefficients) {
    ...
    public MultinomialRegression(RegressionType type, Class<E1> enumType, Map<E1, MultiKeyCoefficientMap> maps,
            boolean preProcessedFlag) {
        if (preProcessedFlag)
            this.maps = maps;
        else
