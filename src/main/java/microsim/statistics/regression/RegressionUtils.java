...
public class RegressionUtils {
    ...
    private static final double EPSILON = 1.e-15; // Consider making larger if there are regular
                                                  // IllegalArgumentException throws due to an unnecessarily high
                                                  // requirement of precision.
    private static final double SYMMETRIC_MATRIX_EPS = 1.e-5; // Relative tolerance of matrix symmetric test, defined as
                                                              // {Mij - Mji > max(Mij, Mji) * eps}
    ...
    /**
     * Returns an event determined randomly from a set of events and weights. Note
     * that these weights do not
     * necessarily have to sum to 1, as the method can convert the weights into
     * probabilities by calculating their relative proportions. If the user desires
     ...
     * @param weights        - an array of doubles from which the probabilities are
     *                       calculated (by dividing the weights by the sum of
     *                       weights).
     * @param rnd            - Random number generator
     * @param checkWeightSum - Boolean toggle, which if true means that the method
     *                       will throw an exception if the weights do not add to 1.
     *                       If false, the method will calculate the probabilities
     *                       associated to the weights and use these in the random
        ...
        for (int i = 0; i < weights.length; i++) {
            if (weights[i] < 0.) {
                throw new IllegalArgumentException(
                        "Negative weights (probabilities) are not allowed!  Check 'weights' array " + weights
                                + " element number " + i + ", which currently has the value " + weights[i] + ".");
            }
            x += weights[i];
        }

        if (checkWeightSum) { // Enforce the condition that the weights must sum to 1 (within some small error
                              // tolerance).
            if (Math.abs(x - 1.0) > EPSILON) // If IllegalArgumentException is too often called, i.e. precision is
                                             // unnecessarily high, consider increasing value of EPSILON
                throw new IllegalArgumentException(
                        "As checkWeightSum is set to true, the probability weights must sum to 1.  The current weights object "
                                + weights + " has elements that sum to " + x
                                + ".  Either ensure probability weights sum to 1, or set checkWeightSum to false so that the weights will be automatically normalised.");
        } else { // Calculate the associated probabilities by dividing the weights by the sum of
                 // weights.
    ...
    public static boolean event(double prob, Random rnd) {
        if (prob < 0. || prob > 1.) {
            throw new IllegalArgumentException(
                    "prob outside the valid interval [0,1] in RegressionUtils.event(prob, rnd)!");
        } else
            return rnd.nextDouble() < prob;
    }

    /**
     * Although this method is easy to use with Maps with events as the keys and
     * probabilities as
     * the values, it will be very slow to call it in a loop, as the events[] and
     * prob[] need to be
     ...
     * {@code event(Map<T, Double> map, Random rnd, boolean checkWeightSum)} method
     * instead, setting {@code checkWeightSum} to false
     * 
     * @param map
     * @param rnd
    ...
    public static <T> T event(Map<T, Double> map, Random rnd) {
        ...
        for (int i = 0; i < events.length; i++) {
            T key = events[i];
            prob[i] = ((Number) map.get(key)).doubleValue(); // This ensures events and prob arrays are aligned by
                                                             // indices.
        }
    ...
    /**
     * 
     * Useful for sampling without replacement whenever the checkWeightSum flag is
     * set to false,
     * as new probabilities are calculated each time this method is called.
     * 
     * If checkWeightSum flag is set to true, which signifies that the map's values
     * are
     * weights that sum to 1, this method will be comparatively slower to call
     * in a loop compared to other available methods, as the events[] and prob[]
     * need to be
     * extracted each time this method is called. Better to use the other method
     * (event(T[], Double[]) and do the extracting of the map outside of the loop!
     * 
     * @param map - contains events as keys and weights as values. These weights
     *            will be normalised to derive probabilities.
     * @param rnd
     * @return the event chosen
     */
    ...
    /**
     * For sampling an event where all events in the sample space have equal
     * probability.
    ...
    /**
     * Performs a linear interpolation on the (numerical) event domain of a
     * piecewise constant probability distribution
     * 
     * @param events : The discrete set of cuts characterising the domain of a
     *               piecewise constant probability distribution
     * @param prob   : The discrete set of probabilities characterising a piecewise
     ...
     * piecewise constant probability distribution
     * 
     * @param events          : The discrete set of cuts characterising the domain
     *                        of a piecewise constant probability distribution
     * @param weights         : The discrete set of probabilities characterising a
     *                        piecewise constant probability distribution
     * @param rnd             : The random number generator
     * @param checkSumWeights : If true, will check weights elements sum to 1,
     *                        otherwise they will be normalised (by dividing each
     *                        element by the sum of the elements).
     * @return : The value of the event drawn from a compact domain
     */
    public static double eventPiecewiseConstant(double[] events, double[] weights, Random rnd,
        ...
        if (checkSumWeights) {
            if (x != 1.0) { // Need total probability to sum to 1.0 exactly, otherwise there is the
                            // possibility that toss > sum of probs.
                throw new IllegalArgumentException(
                        "As checkWeightSum is set to true, the probability weights must sum to 1.  The current weights object "
                                + weights + " has elements that sum to " + x
                                + ".  Either ensure probability weights sum to 1, or set checkWeightSum to false so that the weights will be automatically normalised.");
        ...
        double event = events[i - 1] + (events[i] - events[i - 1]) * ((toss + weights[i - 1] - x) / weights[i - 1]); // The
                                                                                                                     // expression
                                                                                                                     // in
                                                                                                                     // the
                                                                                                                     // last
    ...
    /**
     * 
     * Method to bootstrap regression covariates. This method creates a new set of
     * regression estimates by sampling
     * from a multivariate normal distribution with expected values (means) equal to
     * the 'coefficients' parameter and a covariance matrix
     * as specified.
     * 
     * @param map - A MultiKeyCoefficientMap that contains both regression
     *            coefficients and a corresponding covariance matrix.
     *            map is required to only have one key entry in the
     *            MultiKeyCoefficientMap map's MultiKey with a heading 'REGRESSOR'.
     *            map must also contain a value column with the heading
     *            'COEFFICIENT' and an additional value column for each regressor
     *            (covariate),
     *            in order to represent the (square, symmetric, positive
     *            semi-definite) covariance matrix.
     * 
     * @return a MultiKeyCoefficientMap containing a new set of bootstrapped
     *         regression coefficients
     */
    public static MultiKeyCoefficientMap bootstrap(MultiKeyCoefficientMap map) {
        String[] keys = map.getKeysNames();
        int numRowsInCovarianceMatrix = map.size(); // TODO: Check this is correct AND robust, i.e. if we envisage
                                                    // changing the structure of the .xls files, what impact does that
                                                    // have?
        int regressorColumnIndex = -1;
        if (keys.length > 1) {
            throw new IllegalArgumentException(
                    "There are more conditional keys in the multiKey of the map in RegressionUtils.bootstrap(map).  This cannot currently be handled.  \nThe Stack Trace is "
            ...
            // }
            // }
            // System.out.println("Warning, there are more conditional keys in the multiKey
            // of the map in RegressionUtils.bootstrap(map). Check that the covaraince
            // matrix is represented in the correct way.");
        } else {
            if (keys[0].equals(RegressionColumnNames.REGRESSOR.toString())) {
                regressorColumnIndex = 0;
            } else
                throw new NullPointerException("There is no key named " + RegressionColumnNames.REGRESSOR.toString()
                        + " in the map argument of RegressionUtils.bootstrap(map).  If map was loaded from an Excel spreadsheet, check there is a column with the heading \'"
                        + RegressionColumnNames.REGRESSOR.toString() + "\'.");
        }
        ...
        for (int i = 0; i < numValueColumns; i++) {
            ...
            if (valuesNames[i].equals(RegressionColumnNames.COEFFICIENT.toString())) {
                ...
                indexOfValuesNameMap.put(valuesNames[i], index); // Create consistent column numbering that excludes the
                                                                 // estimates column (the order of columns gets mixed
                                                                 // up)
                index++;
            }
        ...
        if (estimateIndex == -1) {
            throw new NullPointerException("There is no value label named "
                    + RegressionColumnNames.COEFFICIENT.toString()
                    + " in the map argument of RegressionUtils.bootstrap(map).  If map was loaded from an Excel spreadsheet, check there is a column with the heading \'"
                    + RegressionColumnNames.COEFFICIENT.toString() + "\'.");
        }
        ...
        for (MapIterator iterator = map.mapIterator(); iterator.hasNext();) {
            ...
            means[rowIndex] = ((Number) mapValuesRow[estimateIndex]).doubleValue(); // Should throw null pointer
                                                                                    // exception if the RHS returns null
            for (String covariableName : indexOfValuesNameMap.keySet()) {
                int columnIndex = indexOfValuesNameMap.get(covariableName);
                covarianceMatrix[rowIndex][columnIndex] = ((Number) mapValuesRow[valuesMap.get(covariableName)])
                        .doubleValue(); // Should throw null pointer exception if the RHS returns null
            }
        }

        RealMatrix realCovarianceMatrix = new Array2DRowRealMatrix(covarianceMatrix);
        MatrixUtils.checkSymmetric(realCovarianceMatrix, SYMMETRIC_MATRIX_EPS); // Not used as cannot know the
                                                                                // appropriate value of
                                                                                // SYMMETRIC_MATRIX_EPS (relative
                                                                                // tolerance) a priori
        MultivariateNormalDistribution multiNormDist = new MultivariateNormalDistribution(
                (RandomGenerator) SimulationEngine.getRnd(), means, covarianceMatrix);
        means = multiNormDist.sample(); // This returns the bootstrapped values of the estimates

        // Create new multikeycoefficientmap to return with new bootstrapped column, in
        // addition to estimate and standard error data
     ...
     * regression estimates by sampling
     * from a multivariate normal distribution with expected values (means) equal to
     * the 'coefficients' parameter and
     * a covariance matrix as specified.
     * 
     * @param coefficients     - A MultiKeyCoefficientMap that contains a set of
     *                         regression coefficients. The 'coefficients' map
     *                         is required to only have one key entry in each of the
     *                         MultiKeyCoefficientMap map's MultiKeys, with the
     *                         entries being
     *                         the name of the regression covariates. There must be
     *                         only one values column containing the regression
     *                         coefficients
     *                         for each covariate (regressor).
     * 
     * @param covarianceMatrix - A MultiKeyCoefficientMap that provides the
     *                         covariance matrix of a regression's coefficients.
     *                         The covarianceMatrix map is required to only have one
     *                         key entry in each of the MultiKeyCoefficientMap map's
     *                         MultiKeys,
     *                         with each entry corresponding to the name of a
     *                         regression covariate. The values must contain a name
     *                         key corresponding
     *                         to each of the covariates, though the ordering of the
     *                         values (columns) need not match the MultiKey (row)
     *                         ordering.
     * 
     * @return a MultiKeyCoefficientMap of new regression coefficients that is
     *         bootstrapped from the input estimates map.
     */
    public static MultiKeyCoefficientMap bootstrap(MultiKeyCoefficientMap coefficients,
        ...
        if (coefficientKeys.length > 1) {
            throw new IllegalArgumentException(
                    "The estimates map in RegressionUtils.bootstrap(estimates, covarianceMatrix) should only have one "
        ...
        MatrixUtils.checkSymmetric(realCovarianceMatrix, SYMMETRIC_MATRIX_EPS); // Not used as cannot know the
                                                                                ...
                                                                                // tolerance) a priori
        MultivariateNormalDistribution multiNormDist = new MultivariateNormalDistribution(
                (RandomGenerator) SimulationEngine.getRnd(), means, covarianceMatrixOrdered);
        means = multiNormDist.sample(); // This returns the bootstrapped values of the estimates
    ...
    /**
     * Method to bootstrap multinomial regression covariates. This method creates a
     * new map of sets of regression
     * coefficients by sampling from a multivariate normal distribution with
     * expected values (means) equal to the
     * regression coefficients contained in the 'coefficientOutcomeMap' and a
     * covariance matrix as specified.
     * 
     * @param <T>                           The event (outcome) of a multinomial
     *                                      regression
     * 
     * @param eventRegressionCoefficientMap - A map whose keys are the possible
     *                                      events (outcomes) of type T of the
     *                                      multinomial
     *                                      regression, and whose values are
     *                                      MultiKeyCoefficientMaps each containing
     *                                      a set of regression coefficients
     *                                      corresponding to its event (the key).
     *                                      Each MultiKeyCoefficientMap of
     *                                      regression coefficients is used as
     *                                      expected value of a multivariate normal
     *                                      distribution, which is sampled in order
     *                                      to produce a new set of
     *                                      regression coefficients. The
     *                                      MultiKeyCoefficientMaps are required to
     *                                      only have one key entry, with the
     *                                      entries being
     *                                      the name of the regression covariates.
     *                                      There must be only one values column in
     *                                      the MultiKeyCoefficientMaps,
     *                                      containing the regression coefficients
     *                                      for each covariate.
     * 
     * @param covarianceMatrix              - A MultiKeyCoefficientMap that provides
     *                                      the covariance matrix of a regression's
     *                                      coefficients.
     *                                      The covarianceMatrix map is required to
     *                                      only have one key entry in each of the
     *                                      MultiKeyCoefficientMap map's MultiKeys,
     *                                      with each entry corresponding to a sring
     *                                      with the structure [event
     *                                      name]_[covariate name]. So for example,
     *                                      if the set
     *                                      of events of T are LowEducation and
     *                                      HighEducation, and the regression
     *                                      covariates declared in
     *                                      coefficientOutcomeMap
     *                                      are 'age' and 'gender', then the
     *                                      following MultiKey entries must exist:-
     *                                      "LowEducation_age",
     *                                      "LowEducation_gender",
     *                                      "HighEducation_age",
     *                                      "HighEducation_gender", such that the
     *                                      name of the event is a prefix, the
     *                                      character "_" is the
     *                                      'regular expression' and the name of the
     *                                      regression covariate is the suffix. This
     *                                      prefix/suffix ordering must be preserved
     *                                      in order to avoid confusion between the
     *                                      name of events and covariates, however
     *                                      the order in which the MultiKeys are
     *                                      specified
     *                                      does not matter.
     *                                      The values of the MultiKeyCoefficientMap
     *                                      must contain a name key that corresponds
     *                                      to each MultiKey key entry (to ensure
     *                                      labelling of rows and columns match),
     *                                      though the ordering of the values
     *                                      (columns) need not match the MultiKey
     *                                      (row) ordering.
     * 
     * @return a Map whose keys are the possible events (outcomes) of type T of the
     *         multinomial regression and whose values
     *         are MultiKeyCoefficientMap with new regression coefficients (one set
     *         of coefficients for each event).
     */
    public static <T> Map<T, MultiKeyCoefficientMap> bootstrapMultinomialRegression(
            Map<T, MultiKeyCoefficientMap> eventRegressionCoefficientMap, MultiKeyCoefficientMap covarianceMatrix,
            Class<T> enumType) {
        ...
        for (T t : possibleEvents) {
            if (specifiedEvents.contains(t)) {
                ...
                if (count == 0) {
                    baseT = t; // base event which we compare the key names and value names of all other
                               // events' MultiKeyCoefficientMaps
                    ...
                    if (multiKeyMapKeyNames.length != otherKeyNames.length) {
                        throw new IllegalArgumentException(
                                "The number of keys in the regression coefficient MultiKeyCofficientMap for event " + t
                    ...
                    if (map.keySet().size() != covariateMultiKeys.size()) {
                        throw new IllegalArgumentException(
                                "The number of covariates specified in the regression coefficient MultiKeyCofficientMap for event "
                                        + t + " does not match the number of covariates in event " + baseT);
                    }
                    ...
                    for (int i = 0; i < multiKeyMapKeyNames.length; i++) {
                        if (!multiKeyMapKeyNames[i].equals(otherKeyNames[i])) {
                            throw new IllegalArgumentException(
                                    "The key names in the regression coefficient MultiKeyCofficientMap for event " + t
                    ...
                    for (MultiKey mk : otherMultiKeys) {
                        if (!covariateMultiKeys.contains(mk)) {
                            throw new IllegalArgumentException("The covariate " + mk.getKey(0)
                                    ...
                                    + " does not appear in the set of covariates of "
                                    + "event " + baseT
                ...
                missingEvents++;
                if (missingEvents > 1) { // The multinomial regression can go without specifying coefficients for 1 of
                                         // the events (outcomes) of the type T as the probability of this event can be
                                         // determined by the residual of the other probabilities.
        ...
        MultiKeyCoefficientMap enlargedCoefficientMap = new MultiKeyCoefficientMap(multiKeyMapKeyNames,
                multiKeyMapValueNames); // Create new MultiKeyCoefficientMap that has matching key and value names as
        ...
        // throw new IllegalArgumentException("covarianceMatrix contains a key featuring
        //// substrings "
        // + "separated by more than one '_' character. There should only be one
        //// instance of "
        // + "the '_' character in the name keys, which should be used to separate a
        //// prefix "
        // + "corresponding to an event (outcome) name, and a suffix corresponding to a
        //// regression "
        // + "covariate name.");
        // }
     ...
     *
     * @param clazz                   an enum class defining the multinomial
     *                                alternatives considered for analysis
     * @param multinomialCoefficients a standard MultiKeyCoefficientMap object used
     *                                to read parameters from Excel and bootstrap
     *                                The method assumes that coefficients are
     *                                supplied for each (or N-1) discrete
     *                                alternative,
     *                                where "_XXX" is appended to each coefficient
     *                                name to indicate association with alternative
     *                                "XXX"
     * @return multinomialCoeffMap
     * @param <E> Enum object
     */
    public static <E extends Enum<E> & IntegerValuedEnum> Map<E, MultiKeyCoefficientMap> populateMultinomialCoefficientMap(
            Class<E> clazz, MultiKeyCoefficientMap multinomialCoefficients) {
        ...
        if (multinomialCoefficients.getKeysNames().length != 1)
            throw new RuntimeException(
                    "The remapping routine for multinomial regressions is designed for use with single key coefficient maps.");
            ...
            for (String key : regressors) {
                ...
                if (key.endsWith(target)) {
                    ...
                    if (multinomialCoefficients.getValuesNames().length == 1) {
                        ...
                        valHere = ((Number) (multinomialCoefficients.getValue(key, columnName))).doubleValue(); // This
                                                                                                                // allows
                                                                                                                // the
                                                                                                                // prospect
                                                                                                                // of
        ...
        if (added != regressors.size()) {
            // check for terms common to all events (allows for coefficients that satisfy
     ...
     */
    public static MultiKeyCoefficientMap appendCoefficientMaps(MultiKeyCoefficientMap primaryMap,
            MultiKeyCoefficientMap secondaryMap) {
        return appendCoefficientMaps(primaryMap, secondaryMap, null);
    }

    public static MultiKeyCoefficientMap appendCoefficientMaps(MultiKeyCoefficientMap primaryMap,
            MultiKeyCoefficientMap secondaryMap, String secondaryKeyPrefix) {
        return appendCoefficientMaps(primaryMap, secondaryMap, secondaryKeyPrefix, false);
    }

    public static MultiKeyCoefficientMap appendCoefficientMaps(MultiKeyCoefficientMap primaryMap,
            MultiKeyCoefficientMap secondaryMap, String secondaryKeyPrefix, boolean invertSecondary) {

        // check inputs
        if (primaryMap.getKeysNames().length != 1 || secondaryMap.getKeysNames().length != 1)
            throw new RuntimeException(
                    "The routine for appending coefficient maps is designed for use with single key maps.");
        ...
        for (Object keyHere : primaryMap.keySet()) {
            ...
            if (primaryMap.getValuesNames().length == 1) {
                ...
                String columnName = RegressionColumnNames.COEFFICIENT.toString();
                valHere = ((Number) (primaryMap.getValue(keyHere, columnName))).doubleValue(); // This allows the
                                                                                               // prospect of there
                                                                                               // being several value
        ...
        for (Object keyHere : secondaryMap.keySet()) {
            ...
            if (secondaryMap.getValuesNames().length == 1) {
                ...
                String columnName = RegressionColumnNames.COEFFICIENT.toString();
                valHere = ((Number) (secondaryMap.getValue(keyHere, columnName))).doubleValue(); // This allows the
                                                                                                 // prospect of there
                                                                                                 // being several value
                                                                                                 // columns
