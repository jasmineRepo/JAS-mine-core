...
public class RegressionUtils {
    ...
    public static <T> T event(T[] events, double[] weights, Random rnd, boolean checkWeightSum) {
        ...
        for (int i = 0; i < weights.length; i++) {
            if (weights[i] < 0.)
                throw new IllegalArgumentException("Negative weights (probabilities) are not allowed!  Check 'weights' array " + Arrays.toString(weights) + " element number " + i + ", which currently has the value " + weights[i] + ".");
            x += weights[i];
        }
        ...
        if (checkWeightSum) {            //Enforce the condition that the weights must sum to 1 (within some small error tolerance).
            if (Math.abs(x - 1.0) > EPSILON)            //If IllegalArgumentException is too often called, i.e. precision is unnecessarily high, consider increasing value of EPSILON
                throw new IllegalArgumentException("As checkWeightSum is set to true, the probability weights must sum to 1.  The current weights object " + Arrays.toString(weights) + " has elements that sum to " + x + ".  Either ensure probability weights sum to 1, or set checkWeightSum to false so that the weights will be automatically normalised.");
    ...
    public static double eventPiecewiseConstant(double[] events, double[] weights, Random rnd, boolean checkSumWeights) {
        ...
        if (checkSumWeights) {
            //Need total probability to sum to 1.0 exactly, otherwise there is the possibility that toss > sum of probs.
            if (x != 1.0)
                throw new IllegalArgumentException("As checkWeightSum is set to true, the probability weights must sum to 1.  The current weights object " + Arrays.toString(weights) + " has elements that sum to " + x + ".  Either ensure probability weights sum to 1, or set checkWeightSum to false so that the weights will be automatically normalised.");
        } else {                        //Calculate the associated probabilities by dividing the weights by the sum of weights.
            //Convert weights into probabilities by 'normalising' them
            for (int i = 0; i < weights.length; i++) weights[i] /= x;
        }
        ...
        //The expression in the last parentheses is derived from (toss - (x - prob[i-1])) / (x - (x - prob[i-1])), where x is the cumulative probability at the start of events[i] and (x - prob[i-1]) is the cumulative probability at the start of events[i-1]

        return events[i - 1] + (events[i] - events[i - 1]) * ((toss + weights[i - 1] - x) / weights[i - 1]);
    }
    ...
    public static MultiKeyCoefficientMap bootstrap(MultiKeyCoefficientMap map) {
        ...
        if (keys.length > 1) {
            ...
            if (keys[0].equals(RegressionColumnNames.REGRESSOR.toString())) {
                ...
                throw new NullPointerException("There is no key named " + RegressionColumnNames.REGRESSOR + " in the map argument of RegressionUtils.bootstrap(map).  If map was loaded from an Excel spreadsheet, check there is a column with the heading '" + RegressionColumnNames.REGRESSOR + "'.");
        }
        ...
        if (estimateIndex == -1) {
            throw new NullPointerException("There is no value label named " + RegressionColumnNames.COEFFICIENT + " in the map argument of RegressionUtils.bootstrap(map).  If map was loaded from an Excel spreadsheet, check there is a column with the heading '" + RegressionColumnNames.COEFFICIENT + "'.");
        }
                ...
                if (count == 0) {
                    ...
                    for (var mk : otherMultiKeys) {
                        if (!covariateMultiKeys.contains(mk)) {
                            throw new IllegalArgumentException("The covariate " + ((MultiKey<?>) mk).getKey(0) + " specified in the regression coefficient MultiKeyCofficientMap for event " + t + " does not appear in the set of covariates of "
