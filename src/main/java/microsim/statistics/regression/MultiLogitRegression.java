...
public class MultiLogitRegression<T extends Enum<T>> implements MultipleChoiceRegression<T> {
    ...
    public MultiLogitRegression(final @NonNull Map<T, MultiKeyCoefficientMap> maps) {
        this(maps, SimulationEngine.getRnd());
    }

    public MultiLogitRegression(final @NonNull Map<T, MultiKeyCoefficientMap> maps, final @NonNull Random random) {
        this.random = random;
        this.maps = maps;
        int count = 0;
        ...
        for (T event : maps.keySet()) {
            ...
            for (Object covariate : covariateSet) {
                if (count == 0) covariateNames.add(covariate.toString());
                else if (!covariateNames.contains(covariate.toString()) ||
                    covariateNames.size() != covariateSet.size()) {
                    throw new RuntimeException("The covariates specified for each outcome of type T in the " +
                        "MultiLogitRegression object do not match!");
                }
            }
            count++;
        }
    ...
    /**
     * Warning - only use when MultiLogitRegression's maps field has values that are MultiKeyCoefficientMaps with only
     * one key.  This method only looks at the first key of the MultiKeyCoefficientMap field of LinearRegression, so
     * any other keys that are used to distinguish a unique multiKey (i.e. if the first key occurs more than once)
     ...
     * @return
     */
    public double getLogitTransformOfScore(final @NonNull T event, final @NonNull Map<String, Double> values) {
        final double score = LinearRegression.computeScore(maps.get(event), values);
        return (double) 1 / (1 + Math.exp(-score));
    }

    public double getLogitTransformOfScore(final @NonNull T event, final @NonNull Object individual) {
    ...
    public T eventType(final @NonNull Object individual) {
        ...
        for (T eventProb : eventProbs) {
            if (probs.get(eventProb) == null) {
                //The multiLogit regression can go without specifying coefficients for 1 of the outcomes as the
                ...
                if (countNullEventProbs > 1) {
                    throw new RuntimeException("MultiLogitRegression has been constructed with a map that does not " +
                    ...
                    denominator += 0.5;
                    //We include the base case, where score = 0 (as betas are set to zero).
                    // The normalRV.cdf(0) = 0.5 (as the standard normal distribution is symmetric).
                    // The other cases have already been incremented into the denominator.
                    ...
                    // Logit transform (the cumulative standard normal distribution) to the score of 0, and the
                    // denominator is the sum of Logit transforms for all events.
                }
            }
        }
        ...
        for (T event : maps.keySet()) {
            ...
            probs.put(event, LogitTransformOfScoreForEvent / denominator);
            //Normalise the Logit transform of score (the application of the standard normal cumulative distribution to the score) of the event by the sum for all events
        }
    ...
    /**
     * Warning - only use when MultiLogitRegression's maps field has values that are MultiKeyCoefficientMaps with only
     * one key.  This method only looks at the first key of the MultiKeyCoefficientMap field of LinearRegression,
     ...
     * @return
     */
    public T eventType(@NonNull Map<String, Double> values) {
        Map<T, Double> probs = new HashMap<>();
        ...
        for (T eventProb : eventProbs) {
            if (probs.get(eventProb) == null) {
                //The multiLogit regression can go without specifying coefficients for 1 of the outcomes as the probability of this event can be determined by the residual of the other probabilities.
                countNullEventProbs++;
                //Check no more than one event has null prob, so that it is valid to take the residual to find the probability
                if (countNullEventProbs > 1) {
                    throw new RuntimeException("MultiLogitRegression has been constructed with a map that does not" +
    ...
    public <E extends Enum<E>> double getLogitTransformOfScore(final @NonNull T event, final @NonNull DoubleSource iDblSrc, final @NonNull Class<E> Regressors) {
    ...
    public <E extends Enum<E>> T eventType(DoubleSource iDblSrc, Class<E> Regressors, Class<T> enumType) {
        ...
        for (T eventProb : eventProbs) {
            if (probs.get(eventProb) == null) {
                ...
                if (countNullEventProbs > 1) {
                    throw new RuntimeException("MultiLogitRegression has been constructed with a map that does not " +
                        "contain enough of the possible values of the type T.  The map should contain the full " +
                        "number of T values, or one less than the full number of T values (in which case, the missing" +
                        " value is considered the 'default' case whose regression betas are all zero).");
                } else {
                    denominator += 0.5;
                    //We include the base case, where score = 0 (as betas are set to zero).  The normalRV.cdf(0) = 0.5
                    // (as the standard normal distribution is symmetric).  The other cases have already been
        ...
        for (T event : maps.keySet()) {
            ...
            double LogitTransformOfScoreForEvent = probs.get(event);
            probs.put(event, LogitTransformOfScoreForEvent / denominator);
            //Normalise the Logit transform of score (the application of the standard normal cumulative distribution to
            // the score) of the event by the sum for all events
        }
