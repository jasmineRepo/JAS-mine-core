...
public class MovingAverageTraceFunction extends AbstractFunction implements IDoubleSource {
    ...
    public void applyFunction() {
        if (valueCount < len) { // Slower calculation at startup as average is calculated directly by summing
                                // all entries in the values array
            valueCount++; // First time this method is called, valueCount is incremented to 1.

            average = 0.; // Reset value

            for (int i = len - valueCount; i < len - 1; i++) { // First time this method is called, skips for loop. When
                                                               // valueCount == len, i starts from 0.
            ...
            switch (type) {
                case TYPE_DBL:
                    values[len - 1] = dblSource.getDoubleValue(valueID);
                    break;
                case TYPE_FLT:
                    values[len - 1] = fltSource.getFloatValue(valueID);
                    break;
                case TYPE_LNG:
                    values[len - 1] = lngSource.getLongValue(valueID);
                    break;
                case TYPE_INT:
                    values[len - 1] = intSource.getIntValue(valueID);
                    break;
            }

            average += values[len - 1];

            average = average / ((double) valueCount); // Divide by number of values included in calculation, instead of
                                                       // window length (len) which would give moving average values
                                                       // biased towards zero.
