 ...
 * that computes a set of probabilities of the potential outcomes of a given
 * event.
...
public abstract class AbstractMultiProbabilityAlignment<T> {
     ...
     * @param closure     - specifies the method returning the unaligned
     *                    probabilities
     *                    of outcomes for the agent and the method that samples the
     ...
     *                    which the outcomes
     *                    (defined by the AlignmentMultiProbabilityClosure) must be
     *                    true.
     * 
     */
    public abstract void align(Collection<T> agents, Predicate<T> filter, AlignmentMultiProbabilityClosure<T> closure,
            double[] targetShare);
