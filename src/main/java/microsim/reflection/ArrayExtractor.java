...
public class ArrayExtractor {
    ...
    public static double[] extractDoubleArray(Collection<?> collection, Class<?> clazz, String fieldName,
            boolean isMethod) {
    ...
    public static float[] extractFloatArray(Collection<?> collection, Class<?> clazz, String fieldName,
            boolean isMethod) {
        final FloatValueExtractor extractor = new FloatValueExtractor(clazz, fieldName, isMethod);

        final float[] vector = new float[collection.size()];

        int i = 0;
        for (Object element : collection) {
            vector[i++] = extractor.getFloat(element);
        }

        return vector;
    }

    public static int[] extractIntegerArray(Collection<?> collection, Class<?> clazz, String fieldName,
            boolean isMethod) {
    ...
    public static long[] extractLongArray(Collection<?> collection, Class<?> clazz, String fieldName,
            boolean isMethod) {
    ...
    public static String[] extractStringArray(Collection<?> collection, Class<?> clazz, String fieldName,
            boolean isMethod) {
