...
public abstract class Event implements Comparable<Event> {
    ...
    protected int ordering; // If two events have time fields with equal value, their ordering fields will
                            // determine the order in which the events are fired, with lower ordering values
                            // fired before high ordering values. If ordering fields are also equal, the
                            // event that was scheduled first will be fired first in the schedule
                            // (determined comparing the eventNumber field).
    private long eventNumber = eventCounter++; // Designed to break randomness of cases when time and ordering of two
                                               // events is the same. In this case, the first event that was scheduled
                                               // will be fired first in the schedule.
    ...
    public int compareTo(Event e) {
        // Ross Richardson: See Joshua Bloch's Effective Java (2nd Ed.) page 65. "For
        // floating-point fields, use Double.compare..."
        ...
        // time and ordering must be equal, so now check which event was scheduled first
        // and return an int such that the first event is fired first.
        if (eventNumber > e.eventNumber) {
            return 1;
        } else if (eventNumber < e.eventNumber) {
            return -1;
        } else
            throw new RuntimeException("Two events have the same eventNumber.  This should not be possible!\n"
                    + Thread.currentThread().getStackTrace());

        // return 0; //Time, ordering and eventNumber fields are equal - this should not
        // be possible!
