...
/**
 * The eventQueue manages a time ordered queue of events. It is based on a
 * priority
 * queue. At every simulation step the head of the queue is taken and fired.
 * This
...
public class EventQueue {
    ...
    protected Queue<Event> eventQueue; // Ross - changing eventQueue from a linked list to a priority queue in order to
                                       // improve time complexity
    // protected Iterator<Event> iterator;// = eventQueue.iterator(); //Define below
    // - note this is no longer a list iterator!
    ...
    protected void scheduleEvent(Event event) {
        eventQueue.add(event); // Should automatically be fitted into a valid position in the priority queue by
                               // simply using the add method.
    }
     ...
     * @param withOrdering
     *                     The order that the event will be fired: for two events e1
     *                     and e2 scheduled to occur at the same time
     *                     ({@code e1.time == e2.time}), if
     *                     {@code e1.ordering < e2.ordering}, then e1
     *                     will be fired first. If {@code e1.time == e2.time} AND
     *                     {@code e1.ordering == e2.ordering},
     *                     the first event that was scheduled (added to the
     *                     {@code EventQueue}) will be fired first.
     */
     ...
     * 
     * @param atTime
     *                          The time when event will be fired for the first
     *                          time.
     * @param withOrdering
     *                          The order that the event will be fired: for two
     *                          events e1 and e2 scheduled to occur at the same time
     *                          ({@code e1.time == e2.time}), if
     *                          {@code e1.ordering < e2.ordering}, then e1
     *                          will be fired first. If {@code e1.time == e2.time}
     *                          AND {@code e1.ordering == e2.ordering},
     *                          the first event that was scheduled (added to the
     *                          {@code EventQueue}) will be fired first.
     * @param timeBetweenEvents
     *                          The time period between repeated firing of the
     ...
     * - all events scheduled using this method are set with a default ordering of
     * 0.
     * In this instance, if events e1 and e2 are scheduled for the same time (i.e.
     * e1.time == e2.time) using this method,
     * there is no way of ensuring that, for example, e2 is fired before e1 - the
     * actual order these same-time events is
     ...
     * double), where the order of same-time events can be fully specified
     ...
     *                     {@code EventQueue}) will be fired first.
     * @param withLoop
     *                     The time period between repeated firing of the event. If
     *                     this parameter is set to 0, this event will not be fired
     *                     more than once.
     * 
     * @param engine
     * @param type
