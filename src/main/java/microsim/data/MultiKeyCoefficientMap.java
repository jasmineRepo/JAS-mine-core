...
public class MultiKeyCoefficientMap extends MultiKeyMap<Object, Object> {
        ...
        if (key.length == keys.length) {
            ...
            switch (key.length) {
                case 1:
                    throw new IllegalArgumentException("Wrong number of key parameters");
                case 2:
                    if (key[0] instanceof MultiKey) // Ross: If we don't do this check, a new MultiKey of a MultiKey is
                                                    // created unnecessarily, which then leads to a null pointer
                                                    // exception as the MultKeyCoefficientMap does not have a key entry
        ...
        if (keyValues.length == keys.length + 1) {
            switch (keyValues.length) {
                case 1:
                    throw new IllegalArgumentException("Wrong number of key parameters");
                case 2:
                    if (keyValues[0] instanceof MultiKey) // Ross: If we don't do this check, a new MultiKey of a
    ...
    /**
     * This method allows the instance of the MultiKeyCoefficientMap to provide a
     * clone of the names of the keys.
     * This is especially useful for getting the name of the variables used as keys
     * in the Regression classes
     * (in package microsim.statistics.regression)
     * 
     * @return a String array clone of the names of the MultiKeyCoefficientMap's
     *         keys
     */
    public String[] getKeysNames() { // The instance of a MultiKeyCoeffientMap can provide the name of the variables
                                     // used as keys.
    ...
    /**
     * This method allows the instance of the MultiKeyCoefficientMap to provide a
     * clone of the names of the values.
     * This is especially useful for cases where the size of valueColumns > 1, for
     * instance to extract the names
     * of the Type array (T[] events) for RegressionUtils.event(T[] events, double[]
     * prob) method (in package
