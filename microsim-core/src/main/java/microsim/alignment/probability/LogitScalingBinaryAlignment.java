 ...
 * LogitScalingMultiAlignment class instead. 
 * 
...
public class LogitScalingBinaryAlignment<T> extends AbstractProbabilityAlignment<T> {
	...
	public void align(List<T> agentList, Predicate filter, AlignmentProbabilityClosure<T> closure, double targetShare) {
		...
		final int maxNumberIterations = 10000;		//The maximum number of iterations until the iterative loop in the alignment algorithm terminates.  The resulting probabilities at that time are then used.
		final double precision = 1.e-10;				//The appropriate value here depends on the precision of the probabilities.  If the probabilities are stated to x decimal places, then EPSILON should be 1.e-x.
	...
	public void align(List<T> agentList, Predicate filter, AlignmentProbabilityClosure<T> closure, double targetShare, int maxNumberIterations, double precision, boolean enableWarnings) {
		if (targetShare < 0. || targetShare > 1.) {
			System.err.println("target probability in LogitScalingBinaryAlignment.align() method must lie in [0,1]");
			System.exit(0);
		}
		if (maxNumberIterations < 1) {
			System.err.println("maxNumberIterations in LogitScalingBinaryAlignment.align() method must be at least 1");
			System.exit(0);
		} 
		if (precision <= 0.) {
			System.err.println("precision in LogitScalingBinaryAlignment.align() method must be greater than 0");
			System.exit(0);
		}
